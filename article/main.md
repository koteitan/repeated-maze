...
### 9.3 ミンスキー倍加マシン (md, $\Theta(2^k)$ 迷路)
...
§8.2 の評価より、 ブロック内の Haskell rule 数 $p$ に対して経路長は $L^* = \Omega(f(p))$ となる。 ペンテーションの場合 $f(n) = 2 \uparrow\uparrow\uparrow n$ なので

\begin{eqnarray}
L^* = \Omega(2 \uparrow\uparrow\uparrow p)
\end{eqnarray}

すなわち入力 $a$ に対して最短経路長はペンテーションオーダーで爆発する。 ブロックの複雑性 (1 ブロックあたりのマイクロサブブロック数) は $\leq 781\, p^2$ で抑えられる。

#### 9.4.3 規模 (initial_a=1)
