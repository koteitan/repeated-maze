**Japanese** | [English](5.md)

# ミンスキーダブリングマシン k=5

nterm=61, パス長=662, y_final=63

## レジスタマシン

2カウンタ ミンスキーレジスタマシン。アフィン写像 `y ↦ 2y + 1` を
5回反復して `2^6 - 1 = 63` を計算する。
...
```haskell
-- k=5 ×2 ダブリング ミンスキーマシン（繰り返し迷路上）
-- 2レジスタ (x, y)、プログラムカウンタ pc
-- y = 2^6 - 1 = 63 を計算し、y = 1 まで排出
-- 初期呼出: dbl5 (0, 1, 0)
-- 総ステップ数: 662
...
-- 開始
dbl5 (  _,   1,   0) = dbl5 (  1,   1,   1)     -- 入口: x:=1

-- ダブリングサイクル 1: y:1 → 3
dbl5 (  x,   0,   1) = dbl5 (  x,   1,   2)     -- y=0: ブリッジ、P2 に入る
dbl5 (  x,   y,   1) = dbl5 (x+2, y-1,   1)     -- y>0: x+=2, y-=1
dbl5 (  1,   y,   2) = dbl5 (  1,   y,   3)     -- x→0: D2 へ出る
...
-- フェーズ 3: y を排出 63 → 0 → 1
dbl5 (  _,   0,  11) =       (  0,   1,  -1)     -- ゴールで停止 (0,1)
dbl5 (  x,   y,  11) = dbl5 (  x, y-1,  11)     -- y>0: y-=1
```

### 演算

各ダブリングサイクルは2つのフェーズで構成：

| フェーズ | 演算 | 反復ごとの効果 | ゼロテスト |
|---|---|---|---|
| フェーズ1（奇数 pc） | `while y > 0: x += 2, y -= 1` | x が2増加、y が1減少 | y = 0 → ny 境界 |
| フェーズ2（偶数 pc） | `while x > 1: x -= 1, y += 1` | x が1減少、y が1増加 | x = 0 → nx 境界 |

1サイクルの効果: `y_new = 1 + 2 * y_old`

フェーズ3は y を1まで排出し、ゴール状態 (0, 1) で停止する。

### 迷路へのマッピング

| 概念 | レジスタマシン | 繰り返し迷路 |
|---|---|---|
| レジスタ x | INC/DEC x | W→E ポート (x+1) / E→W ポート (x-1) |
| レジスタ y | INC/DEC y | S→N ポート (y+1) / N→S ポート (y-1) |
| ゼロテスト x=0 | JZ x | nx 境界ブロック (x=0) |
| ゼロテスト y=0 | JZ y | ny 境界ブロック (y=0) |
| プログラムカウンタ | pc 値 | ターミナルインデックス (dir, idx) |
| 分岐 | 0でのパターンマッチ | nx/ny と normal ブロックの異なるポート |
...
idx  0    : 開始（E側）
idx  1    : ゴール（E側）
idx  2- 5 : D1 フェーズ1ループ (N2→E3, W3→E4, W4→N5, N5→S2)
idx  7-10 : D1 フェーズ2ループ (S7→E8, E8→W9, E9→N10, S10→E8)
idx 11    : D1 nx キャッチ出力
idx 12-14 : フェーズ3 排出 (N12→S12, N13 ブリッジ, E14→W1)
idx 15-18 : D2 フェーズ1ループ
idx 20-23 : D2 フェーズ2ループ
idx 24    : D2 nx キャッチ出力
idx 27-30 : D3 フェーズ1ループ
idx 32-35 : D3 フェーズ2ループ
idx 36    : D3 nx キャッチ出力
idx 39-42 : D4 フェーズ1ループ
idx 44-47 : D4 フェーズ2ループ
idx 48    : D4 nx キャッチ出力
idx 51-54 : D5 フェーズ1ループ
idx 56-59 : D5 フェーズ2ループ
idx 60    : D5 nx キャッチ出力
```
...
```
T(k) = O(2^k)  （k = ダブリングサイクル数）
nterm = 12k + 3  （概算）
T(nterm) = O(2^{nterm/12})  [nterm に対して指数的]
```

カウンターポンプとの交差点: nterm ≈ 253 (k=21)。
